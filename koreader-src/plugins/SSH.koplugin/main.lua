...
local WATCHDOG_INTERVAL = 30
local WATCHDOG_RETRY_INTERVAL = 8
...
function SSH:init()
    ...
    local autostart_setting = G_reader_settings:readSetting("SSH_autostart")
    self.autostart = autostart_setting == nil and Device:isKobo() or G_reader_settings:isTrue("SSH_autostart")
    if autostart_setting == nil and self.autostart then
        G_reader_settings:makeTrue("SSH_autostart")
    end
    self.ssh_should_run = self.autostart
...
function SSH:_readFile(file_path)
...
function SSH:_isDropbearProcAlive(pid)
    if not pid or not util.pathExists("/proc/" .. pid) then
        return false
    end

    local cmdline = self:_readFile("/proc/" .. pid .. "/cmdline")
    return cmdline == "" or cmdline:find("dropbear", 1, true) ~= nil
end

function SSH:_clearStalePID()
    os.remove(PID_PATH)
end

function SSH:_dropbearCommand(with_keepalive)
    local cmd = string.format("%s %s %s %s%s %s",
        ...
        "-p", self.SSH_port,
        "-P " .. PID_PATH)
    if with_keepalive then
        cmd = string.format("%s %s", cmd, "-K 30 -I 0")
    end
...
function SSH:_unscheduleWatchdog()
    if self.watchdog_task then
        UIManager:unschedule(self.watchdog_task)
        self.watchdog_task = nil
    end
end

function SSH:_scheduleWatchdog(delay)
    if self.watchdog_task then
        return
    end
    self.watchdog_task = function()
        self.watchdog_task = nil
        self:_watchdogTick()
    end
    UIManager:scheduleIn(delay or WATCHDOG_INTERVAL, self.watchdog_task)
end

function SSH:_watchdogEnabled()
    return not self.watchdog_paused and (self.autostart or self.ssh_should_run)
end

function SSH:_watchdogTick()
    if not self:_watchdogEnabled() then
        return
    end
    if self:isRunning() then
        self:_scheduleWatchdog()
    else
        logger.warn("SSH: dropbear is not running; watchdog will restart it")
        self:start(true)
    end
end

function SSH:_ensureRunning(reason)
    if not self:_watchdogEnabled() then
        return
    end
    if self:isRunning() then
    ...
    logger.warn("SSH: restarting dropbear after", reason or "health check")
    self:start(true)
end

function SSH:start(silent)
    if self:isRunning() then
    ...
    local cmd = self:_dropbearCommand(true)
    logger.dbg("[Network] Launching SSH server : ", cmd)
    local ok = os.execute(cmd) == 0
    if not ok then
        logger.warn("SSH: dropbear keepalive flags failed; retrying with conservative arguments")
        cmd = self:_dropbearCommand(false)
        ok = os.execute(cmd) == 0
    end
    if ok then
        self:_scheduleWatchdog()
        ...
        self:_scheduleWatchdog(WATCHDOG_RETRY_INTERVAL)
        if not silent then
            local info = InfoMessage:new{
                    icon = "notice-warning",
...
function SSH:isRunning()
    ...
    if self:_isDropbearProcAlive(pid) then
        return true
    end
    logger.warn("SSH: removing stale dropbear pid file:", PID_PATH)
    self:_clearStalePID()
    return false
end
...
function SSH:stopPlugin(force)
    ...
    send("TERM", pid)
    for _ = 1, 20 do
        if not self:_isDropbearProcAlive(pid) then break end
        ffiutil.sleep(0.1)
    end

    if self:_isDropbearProcAlive(pid) and force then
        send("KILL", pid)
        for _ = 1, 10 do
            if not self:_isDropbearProcAlive(pid) then break end
            ffiutil.sleep(0.1)
    ...
    if not self:_isDropbearProcAlive(pid) then
        self:_clearStalePID()
        return true
    end
...
function SSH:onPrepareUSBMS()
    self.watchdog_paused = true
    local ok, err = self:stopPlugin(true)
    if not ok then
        logger.warn("SSH: force-stop before USBMS failed:", err)
        os.execute("killall -q -TERM dropbear 2>/dev/null || true")
        self:_clearStalePID()
    end
end

function SSH:onResume()
    UIManager:scheduleIn(2, function()
        self:_ensureRunning("resume")
    end)
end

function SSH:onNetworkConnected()
    self:_ensureRunning("network connected")
end
