...
function BB8_mt.__index:blitTo8(dest, dest_x, dest_y, offs_x, offs_y, width, height, setter, set_param)
    ...
    -- Perform block copies only when scanlines are physically contiguous.
    if offs_x == 0 and dest_x == 0 and width == self.w and width == dest.w
            and self.stride == width and dest.stride == width then
...
function BBRGB32_mt.__index:blitToRGB32(dest, dest_x, dest_y, offs_x, offs_y, width, height, setter, set_param)
    ...
    -- Perform block copies only when scanlines are physically contiguous.
    local row_bytes = lshift(width, 2)
    if offs_x == 0 and dest_x == 0 and width == self.w and width == dest.w
            and self.stride == row_bytes and dest.stride == row_bytes then
        -- Single step for contiguous scanlines (on both sides)
        ...
        ffi.copy(dstp, srcp, row_bytes*height)
    else
        -- Scanline per scanline copy
