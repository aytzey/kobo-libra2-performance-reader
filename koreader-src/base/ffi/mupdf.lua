...
local function get_draw_colorspace(ctx, color)
    if not color then
        return M.fz_device_gray(ctx)
    end
    if mupdf.bgr then
        return M.fz_device_bgr(ctx)
    end
    return M.fz_device_rgb(ctx)
end

local function draw_to_blitbuffer(page, draw_context, blitbuffer, offset_x, offset_y)
    local ctm = ffi.new("fz_matrix")
    ...
    local width, height = blitbuffer:getWidth(), blitbuffer:getHeight()
    ...
    local pix = W.mupdf_new_pixmap_with_bbox_and_data(
        page.ctx, get_draw_colorspace(page.ctx, page.doc.color), bbox, nil,
        page.doc.color and 1 or 0, ffi.cast("unsigned char*", blitbuffer.data))
    if pix == nil then merror(page.ctx, "cannot allocate pixmap") end

    run_page(page, pix, ctm)

    if draw_context.gamma >= 0.0 then
        M.fz_gamma_pixmap(page.ctx, pix, draw_context.gamma)
    end

    M.fz_drop_pixmap(page.ctx, pix)
end

local function can_draw_direct(page, blitbuffer)
    local expected_type = page.doc.color and BlitBuffer.TYPE_BBRGB32 or BlitBuffer.TYPE_BB8
    local expected_stride = blitbuffer:getWidth() * (page.doc.color and 4 or 1)
    return blitbuffer:getRotation() == 0
        and blitbuffer:getType() == expected_type
        and tonumber(blitbuffer.stride) == expected_stride
end
...
function page_mt.__index:draw(draw_context, blitbuffer, offset_x, offset_y)
    if can_draw_direct(self, blitbuffer) then
        draw_to_blitbuffer(self, draw_context, blitbuffer, offset_x, offset_y)
        return
    end
...
function page_mt.__index:draw_new(draw_context, width, height, offset_x, offset_y)
    ...
    draw_to_blitbuffer(self, draw_context, bb, offset_x, offset_y)
    return bb
end
