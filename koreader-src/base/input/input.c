...
// Older Kobo sysroots expose struct input_event time as a timeval member,
// while newer Linux headers provide input_event_sec/usec aliases.
...
static inline void set_input_event_time(struct input_event* ev)
{
    struct timeval tv;
...
static inline void set_event_table(lua_State* L, const struct input_event* input)
{
    ...
    lua_pushinteger(L, input->input_event_sec);  // time_t
    lua_rawset(L, -3);                           // time.sec = input.input_event_sec
    lua_pushliteral(L, "usec");
    lua_pushinteger(L, input->input_event_usec);  // suseconds_t
    lua_rawset(L, -3);                            // time.usec = input.input_event_usec
