...
#include <arm_neon.h>
#define LV_HAS_ARM_NEON 1
#else
#define LV_HAS_ARM_NEON 0
#endif
#ifdef LINUX
#include <sys/time.h>
...
static inline bool LVWordHasZeroOrHighByte(uintptr_t word)
{
    const uintptr_t ones = ~(uintptr_t)0 / 0xFFU;
    const uintptr_t high_bits = ones * 0x80U;
    return (word & high_bits) != 0 || ((word - ones) & ~word & high_bits) != 0;
}

static inline bool LVUtf8AsciiCStringLength(const char * s, int &len)
{
    const lUInt8 * p = (const lUInt8 *)s;
    const uintptr_t align_mask = sizeof(uintptr_t) - 1;
    while (((uintptr_t)p & align_mask) != 0) {
        if (!*p) {
            len = (int)((const char *)p - s);
            return true;
        }
        if (*p & 0x80)
            return false;
    ...
    for (;;) {
        ...
        if (LVWordHasZeroOrHighByte(word))
            break;
        p += sizeof(word);
    }
    while (*p) {
...
static inline bool LVUtf8AsciiBufferLength(const char * s, int sz, int &len)
{
    const lUInt8 * p = (const lUInt8 *)s;
    const lUInt8 * end = p + sz;
    const uintptr_t align_mask = sizeof(uintptr_t) - 1;
    while (p < end && ((uintptr_t)p & align_mask) != 0) {
        if (!*p)
            break;
        if (*p & 0x80)
            return false;
    ...
    len = (int)(p - (const lUInt8 *)s);
    return true;
}

#if LV_HAS_ARM_NEON
static inline int LVAsciiToUnicodeNeonCopy(const char * s, lChar32 * p, int len)
{
    int i = 0;
    const lUInt8 * src = (const lUInt8 *)s;
    for (; i + 8 <= len; i += 8) {
        const uint8x8_t bytes = vld1_u8(src + i);
        const uint16x8_t u16 = vmovl_u8(bytes);
        vst1q_u32((uint32_t *)(p + i), vmovl_u16(vget_low_u16(u16)));
        vst1q_u32((uint32_t *)(p + i + 4), vmovl_u16(vget_high_u16(u16)));
    }
    return i;
}
#endif

static inline lString32 LVAsciiToUnicode(const char * s, int len)
{
    if (len <= 0)
        return lString32::empty_str;
    lString32 dst;
    ...
    int i = 0;
#if LV_HAS_ARM_NEON
    i = LVAsciiToUnicodeNeonCopy(s, p, len);
#endif
    for (; i < len; i++) {
        p[i] = (lUInt8)s[i];
    }
...
static inline bool LVUnicodeIsAscii(const lChar32 * s, int count)
{
    for (int i = 0; i < count; i++) {
        if (s[i] & ~0x7F)
            return false;
...
static inline lString8 LVUnicodeAsciiToUtf8(const lChar32 * s, int count)
{
    if (count <= 0)
        return lString8::empty_str;
    lString8 dst;
...
lString32 Utf8ToUnicode( const char * s ) {
    if (!s || !s[0])
      return lString32::empty_str;
    int ascii_len = 0;
    if (LVUtf8AsciiCStringLength(s, ascii_len))
      return LVAsciiToUnicode(s, ascii_len);
...
lString32 Utf8ToUnicode( const char * s, int sz ) {
    if (!s || !s[0] || sz <= 0)
      return lString32::empty_str;
    int ascii_len = 0;
    if (LVUtf8AsciiBufferLength(s, sz, ascii_len))
      return LVAsciiToUnicode(s, ascii_len);
...
lString8 UnicodeToUtf8(const lChar32 * s, int count)
{
    if (count <= 0)
      return lString8::empty_str;
    if (LVUnicodeIsAscii(s, count))
      return LVUnicodeAsciiToUtf8(s, count);
    lString8 dst;
