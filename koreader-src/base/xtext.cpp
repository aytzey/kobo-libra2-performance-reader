...
#include <arm_neon.h>
#define XTEXT_HAS_ARM_NEON 1
#else
#define XTEXT_HAS_ARM_NEON 0
#endif
...
static inline bool xtext_is_ascii_utf8(const char * src, int len) {
    const unsigned char * p = (const unsigned char *)src;
    const unsigned char * end = p + len;

    typedef uintptr_t xtext_word_t;
    const uintptr_t align_mask = sizeof(xtext_word_t) - 1;
    const xtext_word_t high_bits = (~(xtext_word_t)0 / 0xFFU) * 0x80U;

    while (p < end && (((uintptr_t)p) & align_mask)) {
        if (XTEXT_UNLIKELY(*p++ & 0x80U))
            return false;
    }

    while (p + sizeof(xtext_word_t) <= end) {
        xtext_word_t word;
        memcpy(&word, p, sizeof(word));
        if (XTEXT_UNLIKELY(word & high_bits))
            return false;
        p += sizeof(xtext_word_t);
    }
...
static inline void xtext_copy_ascii_to_utf32(const char * src, uint32_t * dst, int len) {
    const unsigned char * s = (const unsigned char *)src;
    int i = 0;
#if XTEXT_HAS_ARM_NEON
    for (; i + 8 <= len; i += 8) {
        const uint8x8_t bytes = vld1_u8(s + i);
        const uint16x8_t u16 = vmovl_u8(bytes);
        vst1q_u32(dst + i, vmovl_u16(vget_low_u16(u16)));
        vst1q_u32(dst + i + 4, vmovl_u16(vget_high_u16(u16)));
    }
#endif
    for (; i + 4 <= len; i += 4) {
        dst[i]     = s[i];
...
public:
    ...
    void setTextFromUTF8String(const char * utf8_text, int utf8_len) {
        if (XTEXT_LIKELY(xtext_is_ascii_utf8(utf8_text, utf8_len))) {
            m_length = utf8_len;
            size_t size = (size_t)m_length + 1; // (one slot more, see above)
            m_text = (uint32_t *)malloc(size * sizeof(*m_text));
            xtext_copy_ascii_to_utf32(utf8_text, m_text, m_length);
            m_text[m_length] = 0;
            m_is_valid = true;
            m_has_rtl = m_para_direction_rtl;
            return;
        }
