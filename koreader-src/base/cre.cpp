...
#include <arm_neon.h>
#define CRE_HAS_ARM_NEON 1
#else
#define CRE_HAS_ARM_NEON 0
#endif

static inline void fixCREColorBuffer(uint8_t * __restrict data, size_t px_count) {
	size_t i = 0;
#if CRE_HAS_ARM_NEON
	for (; i + 8 <= px_count; i += 8) {
		uint8x8x4_t bgra = vld4_u8(data + (i * 4));
		uint8x8x4_t rgba = {{bgra.val[2], bgra.val[1], bgra.val[0], vmvn_u8(bgra.val[3])}};
		vst4_u8(data + (i * 4), rgba);
	}
#endif

	uint8_t * __restrict p = data + (i * 4);
	for (; i < px_count; i++) {
		const uint8_t b = p[0];
		p[0] = p[2];
		p[2] = b;
...
static int drawCurrentPage(lua_State *L) {
	...
	if (color) {
		...
		fixCREColorBuffer(bb->data, (size_t)w * (size_t)h);
	}
	else {
		/* Set DrawBuf to 8bpp */
