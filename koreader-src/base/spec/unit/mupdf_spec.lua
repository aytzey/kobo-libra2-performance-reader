...
describe("mupdf module", function()
    ...
    describe("PDF document API", function()
        ...
        describe("PDF page API", function()
            ...
            it("should draw directly into matching blitbuffers", function()
                local BlitBuffer = require("ffi/blitbuffer")
                local reference = page:draw_new(dc, 160, 120, 0, 0)
                local target = BlitBuffer.new(160, 120, BlitBuffer.TYPE_BB8)

                page:draw(dc, target, 0, 0)

                assert.equals(BlitBuffer.tostring(reference), BlitBuffer.tostring(target))
                reference:free()
                target:free()
            end)
            it("should keep padded blitbuffer rendering equivalent", function()
                local BlitBuffer = require("ffi/blitbuffer")
                local reference = page:draw_new(dc, 160, 120, 0, 0)
                local padded = BlitBuffer.new(160, 120, BlitBuffer.TYPE_BB8, nil, 200, 200)
                local compact = BlitBuffer.new(160, 120, BlitBuffer.TYPE_BB8)

                page:draw(dc, padded, 0, 0)
                compact:blitFrom(padded)

                assert.equals(BlitBuffer.tostring(reference), BlitBuffer.tostring(compact))
                reference:free()
                padded:free()
                compact:free()
            end)
