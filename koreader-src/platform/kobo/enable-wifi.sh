...
for fd in /proc/"$$"/fd/*; do
    ...
    if [ "${fd_id}" -gt 2 ]; then
        if [ "${close_me}" = "true" ]; then
            eval "exec ${fd_id}>&-"
            [ "${KOBO_FD_DEBUG:-0}" = "1" ] && echo "[enable-wifi.sh] Closed fd ${fd_id} -> ${fd_path}"
        else
            ...
            [ "${KOBO_FD_DEBUG:-0}" = "1" ] && echo "[enable-wifi.sh] Left fd ${fd_id} -> ${fd_path} open"
        fi
    fi
done
...
# Keep interactive SSH sessions reachable after idle. Some Kobo Wi-Fi stacks
# aggressively enter client power save after the interface comes up; disable it
# opportunistically without failing bring-up on firmwares lacking these tools.
iwconfig "${INTERFACE}" power off >/dev/null 2>&1 || true
iw dev "${INTERFACE}" set power_save off >/dev/null 2>&1 || true
[ "${WIFI_MODULE}" = "dhd" ] && wlarm_le -i "${INTERFACE}" PM 0 >/dev/null 2>&1 || true

wpa_ctrl_ready() {
    if command -v wpa_cli >/dev/null 2>&1; then
        wpa_cli -i "${INTERFACE}" ping 2>/dev/null | grep -q "PONG" && return 0
    fi
