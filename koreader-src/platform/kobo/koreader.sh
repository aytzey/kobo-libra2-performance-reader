...
ko_libra2_optimize() {
    if [ -r "${KOREADER_DIR}/libra2-optimize.sh" ]; then
        sh "${KOREADER_DIR}/libra2-optimize.sh" "${1}" >>crash.log 2>&1 || true
    fi
}

ko_libra2_preflight() {
    if [ -r "${KOREADER_DIR}/libra2-preflight.lua" ]; then
        ./luajit "${KOREADER_DIR}/libra2-preflight.lua" >>crash.log 2>&1 || true
    fi
}
...
while [ ${RETURN_VALUE} -ne 0 ]; do
    if [ ${RETURN_VALUE} -eq ${KO_RC_RESTART} ]; then
        ...
        ko_do_dns
        # Apply reversible Libra 2 OS-level tuning after the stock Kobo setup.
        ko_libra2_optimize start
        # Seed Libra 2 settings before reader.lua discovers and loads plugins.
        ko_libra2_preflight
    fi
    ...
    if [ ${RETURN_VALUE} -eq ${KO_RC_USBMS} ]; then
        # Restore session tuning before exposing onboard storage via USBMS, then
        # re-apply it on the paths that return to KOReader.
        ko_libra2_optimize stop
...
if [ ${RETURN_VALUE} -ne ${KO_RC_HALT} ]; then
    # Restore values touched by the optional Libra 2 tuning hook before Nickel
    # comes back, so stock power behavior remains in charge outside KOReader.
    ko_libra2_optimize stop
