describe("MenuSorter module", function()
    ...
    it("should allow missing optional empty submenu orders without warning", function()
        ...
        local ok, test_menu = pcall(function()
            return MenuSorter:sort(menu_items, order)
        end)
        logger.warn = old_warn

        assert.is_true(ok)
        assert.is_same({}, warnings)
        assert.is_true(test_menu[1].id == "main")
    end)
