describe("Readerfooter module", function()
    ...
    teardown(function()
        ...
        G_reader_settings:delSetting("libra2_skip_unchanged_footer_update")
    ...
    it("should skip unchanged Libra 2 footer layout work", function()
        G_reader_settings:saveSetting("reader_footer_mode", 1)
        G_reader_settings:saveSetting("libra2_skip_unchanged_footer_update", true)
        local settings = G_reader_settings:readSetting("footer")
        settings.all_at_once = true
        settings.page_progress = true
        settings.pages_left_book = false
        settings.time = false
        ...
        settings.chapter_progress = false
        G_reader_settings:saveSetting("footer", settings)
        ...
        local resets = 0
        local original_reset_layout = footer.horizontal_group.resetLayout
        footer.horizontal_group.resetLayout = function(horizontal_group, ...)
            resets = resets + 1
            return original_reset_layout(horizontal_group, ...)
        end

        footer:onUpdateFooter()
        assert.are.same(0, resets)

        footer:onUpdateFooter(true)
        assert.are.same(1, resets)

        footer.horizontal_group.resetLayout = original_reset_layout
    end)
    ...
    it("should return correct footer height in time mode", function()
        ...
        assert.is.same(
            Screen:scaleBySize(footer.settings.container_height)
                + Screen:scaleBySize(footer.settings.container_bottom_padding),
            footer:getHeight())
    end)
