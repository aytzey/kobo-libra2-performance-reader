describe("FileManager module", function()
    local DataStorage, DocumentRegistry, FileManager, lfs, docsettings, UIManager, Screen, makePath, util
    setup(function()
    ...
    it("detects registered document providers through the Libra 2 fast filter", function()
        assert.is_true(FileManager:hasLibra2FastProvider("book.epub"))
        assert.is_true(FileManager:hasLibra2FastProvider("scan.PDF"))
        assert.is_false(FileManager:hasLibra2FastProvider("notes.notregistered"))
    end)

    it("uses associated document providers but skips auxiliary providers in the fast filter", function()
        local old_provider = G_reader_settings:readSetting("provider")
        local old_aux = DocumentRegistry.known_providers.libra2_test_aux
        DocumentRegistry.known_providers.libra2_test_aux = { provider = "libra2_test_aux", order = 1 }
        G_reader_settings:saveSetting("provider", {
            custombook = "crengine",
            customaux = "libra2_test_aux",
        })

        assert.is_true(FileManager:hasLibra2FastProvider("manual.custombook"))
        assert.is_false(FileManager:hasLibra2FastProvider("notes.customaux"))

        if old_provider == nil then
            G_reader_settings:delSetting("provider")
        else
            G_reader_settings:saveSetting("provider", old_provider)
        end
        DocumentRegistry.known_providers.libra2_test_aux = old_aux
    end)
