describe("device module", function()
    ...
    describe("kobo", function()
        ...
        it("should default Kobo Wi-Fi control socket to wlan0 before the module is loaded", function()
            ...
            util.pathExists.invokes(function(path)
                if path == "/sys/class/net/wlan0" or path == "/sys/class/net/eth0" then
                    return false
                end
                return false
            end)
            ...
            local backend
            local NetworkMgr = {
                sysfsWifiOn = function()
                    return false
                end,
                ifHasAnAddress = function()
                    return false
                end,
                setWirelessBackend = function(_, name, options)
                    backend = {
                        name = name,
                        options = options,
                    }
            ...
            kobo_dev:initNetworkManager(NetworkMgr)

            assert.are.equal("wlan0", NetworkMgr:getNetworkInterfaceName())
            assert.are.equal("wpa_supplicant", backend.name)
            assert.are.equal("/var/run/wpa_supplicant/wlan0", backend.options.ctrl_interface)

            util.pathExists:revert()
        end)

        it("should not use USB ethernet as the Kobo Wi-Fi control interface", function()
            os.getenv.invokes(function(key)
                if key == "PRODUCT" then
                    return "io"
                elseif key == "INTERFACE" then
            ...
            stub(util, "pathExists")
            util.pathExists.invokes(function(path)
                if path == "/sys/class/net/wlan0" then
                    return false
                elseif path == "/sys/class/net/eth0" then
                    return true
                end
                return false
            end)
        ...
        it("should ignore USB gadget INTERFACE when selecting Kobo Wi-Fi control socket", function()
            os.getenv.invokes(function(key)
                if key == "PRODUCT" then
                    return "io"
                elseif key == "INTERFACE" then
