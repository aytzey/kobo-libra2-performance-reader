...
function UIManager:schedule(sched_time, action, ...)
    ...
    local nb_tasks = #task_queue
    if nb_tasks == 0 or sched_time < task_queue[nb_tasks].time then
        task_queue[nb_tasks + 1] = task
        self._task_queue_dirty = true
        return
    end
    if sched_time >= task_queue[1].time then
        table.insert(task_queue, 1, task)
        self._task_queue_dirty = true
        return
    end

    local lo, hi = 1, nb_tasks
    ...
    while lo <= hi do
        ...
        local mid_time = task_queue[mid].time
    ...
    table.insert(task_queue, lo, task)
    self._task_queue_dirty = true
end
...
function UIManager:unschedule(action)
    ...
    local removed = false
    local write = 1
    for read = 1, nb_tasks do
        local task = task_queue[read]
        if task.action == action then
            removed = true
        else
            if write ~= read then
                task_queue[write] = task
            end
            write = write + 1
        end
    end
    if removed then
        for i = write, nb_tasks do
            task_queue[i] = nil
        end
    end
    return removed
...
function UIManager:_checkTasks()
    ...
    while self._task_queue[1] do
        ...
        local task_time = task_queue[task_count].time
        if task_time <= self._now then
            -- Remove the upcoming task, as it is due for execution...
            local task = task_queue[task_count]
            task_queue[task_count] = nil
            -- ...so do it now.
