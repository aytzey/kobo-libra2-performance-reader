...
local EBUSY = 16
local LAST_CONNECTED_SSID_KEY = "__last_connected_ssid"
...
local NetworkMgr = {
    ...
    connectivity_check_interval = 0.25,
    connectivity_check_timeout = 45,
    fast_reconnect_timeout = 5,
    fast_reconnect_max_candidates = 2,
}
...
function NetworkMgr:connectivityCheck(iter, callback, widget)
    local interval = tonumber(self.connectivity_check_interval) or 0.25
    local timeout = tonumber(self.connectivity_check_timeout) or 45
    -- Give up after a while (restoreWifiAsync can take over 45s, so, try to cover that)...
    if iter * interval >= timeout then
        logger.info("Failed to restore Wi-Fi (after", iter * interval, "seconds)!")
    ...
    if self.is_wifi_on and self.is_connected then
        ...
        UIManager:scheduleIn(interval, self.connectivityCheck, self, iter + 1, callback, widget)
    end
end

function NetworkMgr:scheduleConnectivityCheck(callback, widget)
    self.pending_connectivity_check = true
    UIManager:scheduleIn(tonumber(self.connectivity_check_interval) or 0.25, self.connectivityCheck, self, 1, callback, widget)
end
...
function NetworkMgr:startObtainIPAsync() end
...
function NetworkMgr:rememberConnectedNetwork(ssid)
    if not ssid or ssid == "" then
        return
    end
    if not self.nw_settings then self:readNWSettings() end
    self.nw_settings:saveSetting(LAST_CONNECTED_SSID_KEY, ssid)
    local saved = self.nw_settings:readSetting(ssid)
    if type(saved) == "table" and not saved.ssid then
        saved.ssid = ssid
    end
    self.nw_settings:flush()
end

function NetworkMgr:requestIPAsync()
    if self.obtain_ip_async_running then
        return
    end
    self.obtain_ip_async_running = true
    local async_started = self.startObtainIPAsync and self:startObtainIPAsync()
    if not async_started then
        self:obtainIP()
        self.obtain_ip_async_running = false
    end
end

function NetworkMgr:getSavedNetworkCandidates()
    if not self.nw_settings then self:readNWSettings() end
    local candidates = {}
    local seen = {}

    local function addCandidate(ssid, network)
        if type(network) ~= "table" then
            return
        end
        ssid = network.ssid or ssid
        if type(ssid) ~= "string" or ssid == "" or seen[ssid] then
            return
        end
        if network.password == nil and network.psk == nil then
            return
        end
        local candidate = {}
    ...
    local last_ssid = self.nw_settings:readSetting(LAST_CONNECTED_SSID_KEY)
    if last_ssid then
        addCandidate(last_ssid, self.nw_settings:readSetting(last_ssid))
    end
    for ssid, network in pairs(self.nw_settings.data or {}) do
        addCandidate(ssid, network)
    end
    return candidates
end

function NetworkMgr:fastReconnectToSavedNetwork()
    ...
    if current then
        self:queryNetworkState()
        if self.is_connected then
            return true, current.ssid
        end
    end

    local candidates = self:getSavedNetworkCandidates()
    local max_candidates = tonumber(self.fast_reconnect_max_candidates) or 2
    local timeout = tonumber(self.fast_reconnect_timeout) or 5
    local wait_ms = 500
    local max_retry = math.max(2, math.ceil(timeout * 1000 / wait_ms))

    for idx, network in ipairs(candidates) do
        if idx > max_candidates then
            break
        end
        logger.dbg("NetworkMgr: fast reconnect attempt on saved network", util.fixUtf8(network.ssid, "�"))
        ...
        if success then
            self:rememberConnectedNetwork(network.ssid)
            return true, network.ssid
        end
        logger.dbg("NetworkMgr: fast reconnect failed:", err_msg)
    end

    return false
end

function NetworkMgr:startPreferredNetworkWait(complete_callback, info, failure_callback)
    local interval = 0.25
    local max_iter = 60
    self.preferred_network_wait_token = (self.preferred_network_wait_token or 0) + 1
    local token = self.preferred_network_wait_token
    local ip_requested = false
    self.pending_connection = true
    ...
    local function finish(success, ssid, err_msg)
        ...
        self.pending_connection = false
        if success then
            self:rememberConnectedNetwork(ssid)
            if complete_callback then
                complete_callback()
            end
            UIManager:show(InfoMessage:new{
                tag = "NetworkMgr",
                text = T(_("Connected to network %1"), BD.wrap(util.fixUtf8(ssid or _("saved network"), "�"))),
                timeout = 2,
            })
            logger.dbg("NetworkMgr: preferred network connected asynchronously", ssid and util.fixUtf8(ssid, "�") or "<unknown>")
        else
            ...
            logger.dbg("NetworkMgr: preferred network async wait failed:", err_msg)
        end
    end

    local function poll(iter)
        if self.preferred_network_wait_token ~= token then
            return
        end
        local nw = self:getCurrentNetwork()
        if nw then
            if not ip_requested then
                self:requestIPAsync()
                ip_requested = true
            end
            self:queryNetworkState()
        ...
        if iter >= max_iter then
            finish(false, nil, _("Connection timed out"))
            return
        end
...
function NetworkMgr:reconnectOrShowNetworkMenu(complete_callback, interactive)
    local force_scan = self.wifi_toggle_long_press
    local function fallbackToScan(err_msg)
        logger.dbg("NetworkMgr: saved Wi-Fi wait failed, falling back to scan:", err_msg)
    ...
    if not force_scan then
        ...
        if self.is_wifi_on and self.is_connected then
            local current = self:getCurrentNetwork()
            local ssid = current and current.ssid
            if ssid then
                self:rememberConnectedNetwork(ssid)
            end
            if complete_callback then
            ...
            UIManager:show(InfoMessage:new{
                tag = "NetworkMgr",
                text = ssid
                    and T(_("Connected to network %1"), BD.wrap(util.fixUtf8(ssid, "�")))
                    or _("Connected to network"),
                timeout = 2,
            })
            return true
        end

        local fast_info = InfoMessage:new{text = _("Connecting to saved Wi-Fi…")}
        UIManager:show(fast_info)
        UIManager:forceRePaint()
        local fast_success, fast_ssid = self:fastReconnectToSavedNetwork()
        if fast_success then
            logger.dbg("NetworkMgr: Fast reconnected to network", fast_ssid and util.fixUtf8(fast_ssid, "�") or "<unknown>")
            self.wifi_toggle_long_press = nil
            self:startPreferredNetworkWait(complete_callback, fast_info, interactive and fallbackToScan or nil)
            return true
        end
        UIManager:close(fast_info)
    end
    ...
    if Device:hasWifiManager() and not success and not ssid then
        ...
        if has_preferred_networks and not self.wifi_toggle_long_press then
            local wait_info = InfoMessage:new{text = _("Connecting to saved Wi-Fi…")}
            UIManager:show(wait_info)
            self:startPreferredNetworkWait(complete_callback, wait_info, interactive and fallbackToScan or nil)
            self.wifi_toggle_long_press = nil
            return true
        end
    end
    ...
    if success then
        if Device:hasWifiManager() and not Device:isKindle() then
            local wait_info = InfoMessage:new{
                tag = "NetworkMgr",
                text = T(_("Connecting to network %1…"), BD.wrap(util.fixUtf8(ssid, "�"))),
            }
            UIManager:show(wait_info)
            self:startPreferredNetworkWait(complete_callback, wait_info, interactive and fallbackToScan or nil)
            self.wifi_toggle_long_press = nil
