...
function FileChooser:useLibra2MinimalFilemanagerDetails(collate)
    return self.name == "filemanager"
        and collate == self.collates.strcoll
        and G_reader_settings:isTrue("libra2_minimal_filemanager_details")
end

function FileChooser:useLibra2FastFilemanagerScan(collate)
    return self:useLibra2MinimalFilemanagerDetails(collate)
        and G_reader_settings:isTrue("libra2_fast_filemanager_scan")
        and G_reader_settings:readSetting("show_file_in_bold") == false
end

function FileChooser:shouldStatEntryName(name, fast_scan)
    if name == "." or name == ".." then
        return false
    end
    if not FileChooser.show_hidden and util.stringStartsWith(name, ".") then
        return false
    end
    if fast_scan then
        for _, pattern in ipairs(self.exclude_dirs) do
...
local function getAttributes(path, mode_only)
    if mode_only then
        local mode = lfs.attributes(path, "mode")
        return mode and { mode = mode } or {}
    end
    return lfs.attributes(path) or {}
end
...
function FileChooser:getList(path, collate)
    local dirs, files = {}, {}
    local fast_scan = self:useLibra2FastFilemanagerScan(collate)
    -- lfs.dir directory without permission will give error
    local ok, iter, dir_obj = pcall(lfs.dir, path)
    if ok then
        unreadable_dir_content[path] = nil
        for f in iter, dir_obj do
            if self:shouldStatEntryName(f, fast_scan) then
                local fullpath = path.."/"..f
                local attributes = getAttributes(fullpath, fast_scan)
                local item = true
...
function FileChooser:getListItem(dirpath, f, fullpath, attributes, collate)
    local minimal_details = self:useLibra2MinimalFilemanagerDetails(collate)
    local item = {
        text = f,
        path = fullpath,
        attr = attributes,
    ...
    if attributes.mode == "file" then
        ...
        if item.text == "./." then -- added as content of an unreadable directory
            ...
            if dirpath and not minimal_details then -- file browser or PathChooser
...
local function getFastFilemanagerSortKey(item)
    if item.libra2_sort_key == nil then
        item.libra2_sort_key = string.lower(item.text or "")
    end
    return item.libra2_sort_key
end

function FileChooser:getSortingFunction(collate, reverse_collate, collate_id)
    local sorting
    ...
    if collate_id == "strcoll" and G_reader_settings:isTrue("libra2_fast_filemanager_sort") then
        sorting = function(a, b)
            local a_key = getFastFilemanagerSortKey(a)
            local b_key = getFastFilemanagerSortKey(b)
            if a_key == b_key then
...
function FileChooser:genItemTableFromPath(path)
    local collate, collate_id = self:getCollate()
    local dirs, files = self:getList(path, collate)
    return self:genItemTable(dirs, files, path, collate, collate_id)
end

function FileChooser:genItemTable(dirs, files, path, collate, collate_id)
    if not collate then
        collate, collate_id = self:getCollate()
    end
    local collate_mixed = G_reader_settings:isTrue("collate_mixed")
    local reverse_collate = G_reader_settings:isTrue("reverse_collate")
    local sorting = self:getSortingFunction(collate, reverse_collate, collate_id)
    ...
    if collate.can_collate_mixed and collate_mixed then
        ...
        if not collate.can_collate_mixed then -- keep folders sorted by name not reversed
            sorting = self:getSortingFunction(self.collates.strcoll, false, "strcoll")
        end
...
function FileChooser:getMenuItemMandatory(item, collate)
    ...
    if collate then -- file
        ...
        if getReadCollection():isFileInCollections(item.path) then
            text = "☆ " .. text
        end
        ...
        if getFileManagerShortcuts():hasFolderShortcut(item.path) then
            text = "☆ " .. text
        end
    end
    return text
