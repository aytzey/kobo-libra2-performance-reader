...
local function rebuildOrderedTouchZoneCache(self)
    self._ordered_touch_zones = {}
    self._ordered_touch_zones_by_gesture = {}
    if not self.touch_zone_dg then return end

    local serialized_zones = self.touch_zone_dg:serialize()
    for i = 1, #serialized_zones do
        local zone_id = serialized_zones[i]
        local tzone = self._zones[zone_id]
        if tzone then
            self._ordered_touch_zones[#self._ordered_touch_zones + 1] = tzone
            local ges = tzone.gs_range.ges
            local gesture_zones = self._ordered_touch_zones_by_gesture[ges]
            if not gesture_zones then
                gesture_zones = {}
                self._ordered_touch_zones_by_gesture[ges] = gesture_zones
            end
            gesture_zones[#gesture_zones + 1] = tzone
        end
    end
end
...
function InputContainer:registerTouchZones(zones)
    ...
    for i = 1, #zones do
        ...
        if zone.overrides then
            for j = 1, #zone.overrides do
                local override_zone_id = zone.overrides[j]
    ...
    -- print("ordering:")
    rebuildOrderedTouchZoneCache(self)
end
    ...
    if self.touch_zone_dg then
        for i = 1, #zones do
            ...
            if self._zones[zone.id] then
                ...
                for j = 1, #self._ordered_touch_zones do
                    if self._ordered_touch_zones[j].def.id == zone.id then
                        table.remove(self._ordered_touch_zones, j)
                        break
                    end
                end
            end
...
function InputContainer:updateTouchZonesOnScreenResize(new_screen_dimen)
    for i = 1, #self._ordered_touch_zones do
        local tzone = self._ordered_touch_zones[i]
...
function InputContainer:onGesture(ev)
    local ordered_touch_zones = self._ordered_touch_zones_by_gesture[ev.ges]
    if ordered_touch_zones then
        for i = 1, #ordered_touch_zones do
            local tzone = ordered_touch_zones[i]
