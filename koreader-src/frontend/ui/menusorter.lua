...
function MenuSorter:sort(item_table, order)
    ...
    for order_id, order_item in pairs(order) do
        ...
        if item_table[order_id] ~= nil then
            ...
            local optional_empty_menu = type(order_item) == "table" and next(order_item) == nil
            if order_id ~= "KOMenu:disabled" and order_id ~= "plus_menu" and not optional_empty_menu then
