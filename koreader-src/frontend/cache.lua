...
function Cache:check(key, ItemClass)
    ...
    elseif ItemClass then
        local cached = self.cached[xxh3_64_hex(key)] or self.cached[md5(key)]
        if cached then
...
-- Evict stale cache entries if the process is redlining on free RAM.
function Cache:memoryPressureCheck(opts)
    opts = opts or {}
    local now = os.time()
    local min_interval = opts.min_interval or 1
    if not opts.force and self._last_memory_pressure_check and min_interval > 0
            and now - self._last_memory_pressure_check < min_interval then
        return self._last_memory_pressure_state
    end
    ...
    if memfree == nil or memtotal == nil or memtotal <= 0 then
        self._last_memory_pressure_state = nil
        return
    end

    local low_threshold = opts.low_threshold or 0.20
    local critical_threshold = opts.critical_threshold or 0.12
    local free_fraction = memfree / memtotal
    local state = {
        memfree = memfree,
        memtotal = memtotal,
        free_fraction = free_fraction,
        pressured = free_fraction < low_threshold,
        critical = free_fraction < critical_threshold,
    }
    self._last_memory_pressure_state = state

    -- If less than 20% of the total RAM is free, drop half the cache.
    -- Below the critical line, chop once more while keeping the MRU quarter.
    if state.pressured then
        logger.warn(string.format("Running low on memory (~%d%%, ~%.2f/%d MiB), evicting cache entries...",
                                  free_fraction * 100,
                                  memfree / (1024 * 1024),
                                  memtotal / (1024 * 1024)))
        self.cache:chop()
