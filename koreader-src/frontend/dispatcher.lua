...
local lookup_modules_enabled = not (
    G_reader_settings and G_reader_settings:isTrue("libra2_disable_lookup_modules"))

local function getDictionaryPresets()
    ReaderDictionary = ReaderDictionary or require("apps/reader/modules/readerdictionary")
    return ReaderDictionary.getPresets()
end
...
local settingsList = {
    ...
    ----
    dictionary_lookup = {category="none", event="ShowDictionaryLookup", title=_("Dictionary lookup"), general=true, condition=lookup_modules_enabled},
    load_dictionary_preset = {category="string", event="LoadDictionaryPreset", title=_("Load dictionary preset"), args_func=getDictionaryPresets, general=true, condition=lookup_modules_enabled},
    cycle_dictionary_preset = {category="none", event="CycleDictionaryPresets", title=_("Cycle through dictionary presets"), general=true, condition=lookup_modules_enabled},
    wikipedia_lookup = {category="none", event="ShowWikipediaLookup", title=_("Wikipedia lookup"), general=true, condition=lookup_modules_enabled, separator=true},
    ----
...
function Dispatcher.getDisplayList(settings, for_sorting)
    ...
    local labels = settings.settings and settings.settings.labels
    for item, v in Dispatcher.iter_func(settings) do
        ...
        if settingsList[item] ~= nil and settingsList[item].condition ~= false then
            table.insert(item_table, {
                text = labels and labels[item] or Dispatcher:getNameFromItem(item, settings),
                key = item,
...
function Dispatcher.getQuickMenuMinWidth(settings)
    local width_factor = settings and settings.settings and settings.settings.quickmenu_min_width_factor
    width_factor = tonumber(width_factor) or 0.6
    width_factor = math.max(0.35, math.min(0.9, width_factor))
    return math.floor(width_factor * Screen:getWidth())
end
...
function Dispatcher._showAsMenu(settings, exec_props)
    ...
    quickmenu = ButtonDialog:new{
        ...
        shrink_min_width = Dispatcher.getQuickMenuMinWidth(settings),
...
function Dispatcher:execute(settings, exec_props)
    ...
    for k, v in Dispatcher.iter_func(settings, true) do
        ...
        local action = settingsList[k]
        if Dispatcher:isActionEnabled(action) then
            if action.configurable then
                local value = v
                if type(v) ~= "number" then
                    for i, r in ipairs(action.args) do
                        if v == r then value = action.configurable.values[i] break end
                    end
                end
                UIManager:sendEvent(Event:new("ConfigChange", action.configurable.name, value))
            end
            local category = action.category
            local event = action.event
            local arg = action.arg
