...
local DocumentRegistry = {
    ...
    providers = {},
    provider_ext_index = {},
    provider_suffix_index = {},
    provider_ext_registered = {},
...
function DocumentRegistry:addProvider(extension, mimetype, provider, weight)
    ...
    table.insert(self.providers, provider_entry)
    local ext_providers = self.provider_ext_index[extension]
    if ext_providers then
        ext_providers[#ext_providers + 1] = provider_entry
    else
        self.provider_ext_index[extension] = { provider_entry }
    end
    if not self.provider_ext_registered[extension] then
        local last_suffix = extension:match("[^.]+$") or extension
        local suffix_extensions = self.provider_suffix_index[last_suffix]
        if suffix_extensions then
            suffix_extensions[#suffix_extensions + 1] = extension
            table.sort(suffix_extensions, function(a, b) return #a > #b end)
        else
            self.provider_suffix_index[last_suffix] = { extension }
        end
        self.provider_ext_registered[extension] = true
    end
...
local function addProviderCandidate(providers, provider_entry)
    local added = false
    for i = #providers, 1, -1 do
        local prov_prev = providers[i]
        if prov_prev.provider == provider_entry.provider then
            if prov_prev.weight >= provider_entry.weight then
                added = true
            else
    ...
    -- Keep the historical ordering policy: only a new highest weight jumps
    -- to the front; otherwise registration order is preserved.
    if not added and #providers >= 1 and provider_entry.weight > providers[1].weight then
        table.insert(providers, 1, provider_entry)
    elseif not added then
        providers[#providers + 1] = provider_entry
    end
end

function DocumentRegistry:getProviderExtensions(file)
    local file_lower = file:lower()
    local last_suffix = getSuffix(file_lower)
    local suffix_extensions = self.provider_suffix_index[last_suffix]
    if not suffix_extensions then
        return
    end
    local extensions
    for i = 1, #suffix_extensions do
        local extension = suffix_extensions[i]
        if file_lower:sub(-#extension - 1) == "." .. extension then
            if extensions then
...
function DocumentRegistry:getProviders(file)
    ...
    local extensions = self:getProviderExtensions(file)
    if extensions then
        for i = 1, #extensions do
            local ext_providers = self.provider_ext_index[extensions[i]]
            for j = 1, #ext_providers do
                addProviderCandidate(providers, ext_providers[j])
            end
        end
    end
...
local function registerLazyImageViewer(registry)
    registry:addAuxProvider({
        ...
        callback = function(file)
            return require("ui/widget/imageviewer").openFile(file)
        end,
        disable_file = true,
        disable_type = false,
    })
...
local function registerLazyTextViewer(registry)
    registry:addAuxProvider({
        ...
        callback = function(file)
            return require("ui/widget/textviewer").openFile(file)
        end,
        disable_file = true,
        disable_type = false,
    })
...
-- auxiliary built-in
registerLazyImageViewer(DocumentRegistry)
registerLazyTextViewer(DocumentRegistry)
