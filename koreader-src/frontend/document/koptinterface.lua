...
-- Keep this sorted exactly like Configurable:hash() / ffiUtil.orderedPairs().
-- KOPT context hashes are built during PDF reflow, crop, textbox and tile cache
-- paths, so avoid sorting the same stable option set for every page operation.
local kopt_config_hash_keys = {
    "auto_straighten",
    "contrast",
    "defect_size",
    "detect_indent",
    "disable_ocr_fallback",
    "doc_language",
    "font_fine_tune",
    "font_size",
    "forced_ocr",
...
local kopt_config_hash_key_set = {}
for _, key in ipairs(kopt_config_hash_keys) do
    kopt_config_hash_key_set[key] = true
end

local function appendConfigurableHash(configurable, hash_list)
    for key, value in pairs(configurable) do
        local value_type = type(value)
        if (value_type == "number" or value_type == "string")
                and not kopt_config_hash_key_set[key] then
    ...
    local len = #hash_list
    for i = 1, #kopt_config_hash_keys do
        local value = configurable[kopt_config_hash_keys[i]]
        local value_type = type(value)
...
KoptInterface._appendConfigurableHash = appendConfigurableHash
...
function KoptInterface:isOCRFallbackEnabled(doc)
...
function KoptInterface:setDefaultConfigurable(configurable)
    ...
    configurable.disable_ocr_fallback = G_defaults:readSetting("DKOPTREADER_DISABLE_OCR_FALLBACK") and 1 or 0
...
function KoptInterface:waitForContext(kc)
    ...
    local waited = false
    local wait_us = math.max(1000, tonumber(G_defaults:readSetting("DKOPTREADER_BACKGROUND_WAIT_US")) or 100000)
    while kc and kc:isPreCache() == 1 do
        waited = true
        logger.dbg("waiting for background rendering")
        FFIUtil.usleep(wait_us)
    end
...
function KoptInterface:getContextHash(doc, pageno, bbox, hash_list)
    ...
    appendConfigurableHash(doc.configurable, hash_list)
...
function KoptInterface:renderReflowedPage(doc, pageno, rect, zoom, rotation)
    ...
    local cached = DocCache:check(hash)
    if not cached then
        DocCache:memoryPressureCheck{ min_interval = 0 }
        -- do the real reflowing if kctx has not been cached yet
        local kc = self:getCachedContext(doc, pageno)
        local fullwidth, fullheight = kc:getPageDim()
        if not DocCache:willAccept(fullwidth * fullheight) then
            -- whole page won't fit into cache
            logger.warn("Skipping reflowed page render because the tile is too large for cache")
            return nil
        end
...
function KoptInterface:renderOptimizedPage(doc, pageno, rect, zoom, rotation, hinting)
    ...
    if not cached then
        local pressure = DocCache:memoryPressureCheck{ min_interval = 0 }
        if pressure and pressure.critical and hinting then
            logger.dbg("Skipping optimized page hint under critical memory pressure")
            return nil
        end
...
function KoptInterface:getTextBoxes(doc, pageno)
    ...
    if text and #text > 1 and (doc.configurable.forced_ocr ~= 1 or not self:isOCRFallbackEnabled(doc)) then
        return text
    elseif not self:isOCRFallbackEnabled(doc) then
        return text
...
function KoptInterface:getReflowedTextBoxesFromScratch(doc, pageno)
    if not self:isOCRFallbackEnabled(doc) then return nil end
