...
function Document:renderPage(pageno, rect, zoom, rotation, gamma, hinting)
    ...
    if is_prescaled then
        ...
        size = page_size
        local full_page_bytes = size.w * size.h * (self.render_color and 4 or 1) + 512
        if pressure and pressure.critical then
            logger.dbg("Memory pressure is critical before rendering page", pageno)
        end
        if pressure and pressure.critical or not DocCache:willAccept(full_page_bytes) then
            -- ...and if it doesn't...
            ...
            is_full_page_tile = true
        end
    end

    -- Prepare our BB, and wrap it in a cache item for DocCache
    tile = TileCacheItem:new{
        persistent = is_full_page_tile, -- we don't want to dump page fragments to disk (unnecessary, and it would confuse DocCache's heuristics)
        doc_path = self.file,
        created_ts = os.time(),
        excerpt = size,
        pageno = pageno,
