...
local function koboEnableWifi(toggle)
    if toggle == true then
        ...
        os.execute("./enable-wifi.sh >/tmp/koreader-enable-wifi.log 2>&1")
    else
        logger.info("Kobo Wi-Fi: disabling Wi-Fi")
        os.execute("./disable-wifi.sh >/tmp/koreader-disable-wifi.log 2>&1")
    end
end
...
function Kobo:init()
    ...
    self.input = require("device/input"):new{
        ...
        event_map = {
            ...
            [90] = "LightButton",
            [102] = "Home",
            [113] = "HeadsetMute",        -- KEY_MUTE, Bluetooth AVRCP
            [114] = "HeadsetVolumeDown",  -- KEY_VOLUMEDOWN, Bluetooth AVRCP
            [115] = "HeadsetVolumeUp",    -- KEY_VOLUMEUP, Bluetooth AVRCP
            [116] = "Power",
            [119] = "HeadsetPause",       -- KEY_PAUSE, Bluetooth AVRCP
            [163] = "HeadsetNext",        -- KEY_NEXTSONG, Bluetooth AVRCP
            [164] = "HeadsetPlayPause",   -- KEY_PLAYPAUSE, Bluetooth AVRCP
            [165] = "HeadsetPrevious",    -- KEY_PREVIOUSSONG, Bluetooth AVRCP
            [166] = "HeadsetStop",        -- KEY_STOPCD, Bluetooth AVRCP
            [200] = "HeadsetPlay",        -- KEY_PLAYCD, Bluetooth AVRCP
            [201] = "HeadsetPause",       -- KEY_PAUSECD, Bluetooth AVRCP
            [207] = "HeadsetPlay",        -- KEY_PLAY, Bluetooth AVRCP
            [193] = "RPgBack",
            [194] = "RPgFwd",
            [331] = "Eraser",
            [332] = "Highlighter",
    ...
    -- We care about: the touchscreen, the stylus, the power button, the sleep cover,
    -- pagination buttons, and AVRCP volume/media buttons from Bluetooth headphones.
    ...
    local match_mask = bit.bor(C.INPUT_TOUCHSCREEN, C.INPUT_TABLET, C.INPUT_POWER_BUTTON, C.INPUT_SLEEP_COVER, C.INPUT_PAGINATION_BUTTONS, C.INPUT_VOLUME_BUTTONS)
...
local function getKoboWifiInterface()
    local env_interface = os.getenv("INTERFACE")
    if env_interface and env_interface:match("^wlan%d+$") then
        return env_interface
    end
    -- Libra 2 can expose USB networking as eth0/usb0 before the Wi-Fi module
    -- creates wlan0. Treating that as the Wi-Fi interface makes the WPA backend
    -- look for /var/run/wpa_supplicant/eth0 while enable-wifi.sh correctly
...
function Kobo:initNetworkManager(NetworkMgr)
    ...
    function NetworkMgr:turnOnWifi(complete_callback, interactive)
        if self:isWifiOn() then
            logger.info("Kobo Wi-Fi: already enabled, skipping hardware bringup")
        else
            koboEnableWifi(true)
        end
    ...
    local net_if = getKoboWifiInterface()
    ...
    function NetworkMgr:obtainIP()
        os.execute("./obtain-ip.sh >/tmp/koreader-obtain-ip.log 2>&1")
    end

    function NetworkMgr:startObtainIPAsync()
        os.execute("sh -c './obtain-ip.sh >/tmp/koreader-obtain-ip.log 2>&1 &'")
        return true
    end

    function NetworkMgr:releaseIP()
        os.execute("./release-ip.sh >/tmp/koreader-release-ip.log 2>&1")
    end

    function NetworkMgr:restoreWifiAsync()
        os.execute("./restore-wifi-async.sh >/tmp/koreader-restore-wifi.log 2>&1")
    end

    NetworkMgr.isWifiOn = NetworkMgr.sysfsWifiOn
    NetworkMgr.isConnected = NetworkMgr.ifHasAnAddress
    NetworkMgr.connectivity_check_interval = 0.20
    NetworkMgr.connectivity_check_timeout = 24
    NetworkMgr.fast_reconnect_timeout = 4
    NetworkMgr.fast_reconnect_max_candidates = 2
