...
function Input:handleKeyBoardEv(ev)
    ...
    elseif ev.value == KEY_REPEAT then
        ...
        if keycode == "Up" or keycode == "Down" or keycode == "Left" or keycode == "Right"
         or keycode == "RPgBack" or keycode == "RPgFwd" or keycode == "LPgBack" or keycode == "LPgFwd"
         or keycode == "HeadsetVolumeDown" or keycode == "HeadsetVolumeUp"
         or keycode == "HeadsetPrevious" or keycode == "HeadsetNext" then
...
function Input:_gestureEvent(touch_ges)
    self:gestureAdjustHook(touch_ges)
    return Event:new("Gesture", self.gesture_detector:adjustGesCoordinate(touch_ges))
end

function Input:_handleTouchFrame(ev)
    local MTSlots = self.MTSlots
    local slot_count = #MTSlots
    if slot_count == 0 then
        self:newFrame()
        return
    end

    local timev = time.timeval(ev.time)
    if slot_count == 1 then
        local MTSlot = MTSlots[1]
        self:setMtSlot(MTSlot.slot, "timev", timev)
        local touch_ges = self.gesture_detector:feedOneEvent(MTSlot)
        self:newFrame()
        if touch_ges then
            return { self:_gestureEvent(touch_ges) }
        end
        return
    end

    for i = 1, slot_count do
        local MTSlot = MTSlots[i]
        self:setMtSlot(MTSlot.slot, "timev", timev)
    end
    local touch_gestures = self.gesture_detector:feedEvent(MTSlots)
    self:newFrame()

    local gesture_count = #touch_gestures
    if gesture_count == 0 then
        return
    end
    local ges_evs = {}
    for i = 1, gesture_count do
        ges_evs[i] = self:_gestureEvent(touch_gestures[i])
    end
    return ges_evs
end
...
function Input:handleTouchEv(ev)
    ...
    elseif ev.type == C.EV_SYN then
        if ev.code == C.SYN_REPORT then
            return self:_handleTouchFrame(ev)
        end
    end
end
        ...
        for i = 1, #ev do
            ...
            if event.type == C.EV_KEY then
                ...
                if handled_ev then
                    ...
                    handled[handled_count] = handled_ev
                end
            elseif event.type == C.EV_ABS or event.type == C.EV_SYN then
                ...
                if handled_evs then
                    for j = 1, #handled_evs do
                        handled_count = handled_count + 1
                        handled[handled_count] = handled_evs[j]
                    end
                end
            elseif event.type == C.EV_MSC then
