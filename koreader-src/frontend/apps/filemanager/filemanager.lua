...
local function getFileManagerConverter()
    FileManagerConverter = FileManagerConverter or require("apps/filemanager/filemanagerconverter")
    return FileManagerConverter
end
...
local function getMultiConfirmBox()
    MultiConfirmBox = MultiConfirmBox or require("ui/widget/multiconfirmbox")
...
local function getReadHistory()
    ReadHistory = ReadHistory or require("readhistory")
    return ReadHistory
end
...
function FileManager:hasLibra2FastProvider(filename)
    local suffix = util.getFileNameSuffix(filename):lower()
    if DocumentRegistry.filetype_provider[suffix] then
        return true
    end

    local provider_key = G_reader_settings:readSetting("provider", {})[suffix]
    local provider = provider_key and DocumentRegistry.known_providers[provider_key]
    return provider and not provider.order and true or false
end
...
function FileManager:setupLayout()
    ...
    local file_chooser = FileChooser:new{
        ...
        file_filter = function(filename)
            if G_reader_settings:isTrue("libra2_fast_filemanager_filter") then
                return self:hasLibra2FastProvider(filename)
            end
    ...
    function file_chooser:showFileDialog(item)
        ...
        if is_file then
            ...
            local been_opened = getBookList().hasBookBeenOpened(file)
            ...
            if has_provider or been_opened then
                ...
                if been_opened then
                    ...
                    if not book_props then
                        ...
                        book_props = getFileManagerBookInfo().extendProps(props, file)
            ...
            if getFileManagerConverter():isSupported(file) then
                table.insert(buttons, {
                    getFileManagerConverter():genConvertButton(file, close_dialog_callback, refresh_callback)
                })
            end
            if been_opened then
...
function FileManager:init()
    ...
    self:registerModule("history", getFileManagerHistory():new{ ui = self })
    self:registerModule("bookinfo", getFileManagerBookInfo():new{ ui = self })
    self:registerModule("collections", getFileManagerCollection():new{ ui = self })
    self:registerModule("filesearcher", getFileManagerFileSearcher():new{ ui = self })
    self:registerModule("folder_shortcuts", getFileManagerShortcuts():new{ ui = self })
    self:registerModule("languagesupport", getLanguageSupport():new{ ui = self })
    ...
    self:registerModule("devicestatus", getReaderDeviceStatus():new{ ui = self })
    self:registerModule("devicelistener", DeviceListener:new{ ui = self })
    self:registerModule("networklistener", getNetworkListener():new{ ui = self })
...
function FileManager:onShowPlusMenu()
    ...
    self.plus_dialog = getButtonDialog():new{
        title = title,
        title_align = "center",
        buttons = buttons,
    }
...
function FileManager:openRandomFile(dir, unopened_only)
    local match_func = function(file)
        if unopened_only then
            return DocumentRegistry:hasProvider(file) and not getBookList().hasBookBeenOpened(file)
        else
    ...
    if random_file then
        UIManager:show(getMultiConfirmBox():new{
    ...
    local function doPaste()
        ...
        if ok then
            ...
            if self.cutfile then -- for move only
                if is_file then
                    getReadHistory():updateItem(orig_file, dest_file)
                    getReadCollection():updateItem(orig_file, dest_file)
                else
                    getReadHistory():updateItemsByPath(orig_file, dest_file)
                    getReadCollection():updateItemsByPath(orig_file, dest_file)
                end
            end
            self.clipboard = nil
            self:onRefresh()
...
function FileManager:showCopyMoveSelectedFilesDialog(close_callback)
    ...
    check_button_overwrite = getCheckButton():new{
...
function FileManager:pasteSelectedFiles(overwrite)
    ...
    if util.tableSize(ok_files) > 0 then
        if self.cutfile then -- for move only
            getReadHistory():updateItems(ok_files, dest_path)
            getReadCollection():updateItems(ok_files, dest_path)
        end
...
function FileManager:createFolder()
    ...
    check_button_enter_folder = getCheckButton():new{
...
function FileManager:showDeleteFileDialog(filepath, post_delete_callback, pre_delete_callback)
    ...
    if is_file and getBookList().hasBookBeenOpened(file) then
...
function FileManager:deleteFile(file, is_file)
    if is_file then
        ...
        if ok then
            getBookList().resetBookInfoCache(file)
            DocSettings.updateLocation(file) -- delete sdr
            getReadHistory():fileDeleted(file)
            getReadCollection():removeItem(file)
            return true
        end
    else
        local ok = ffiUtil.purgeDir(file)
        if ok then
            getReadHistory():folderDeleted(file) -- will delete sdr
            getReadCollection():removeItemsByPath(file)
            return true
        end
    end
    UIManager:show(getInfoMessage():new{
...
function FileManager:deleteSelectedFiles()
    ...
    for orig_file in pairs(self.selected_files) do
        ...
        if ok then
            getBookList().resetBookInfoCache(file_abs_path)
    ...
    local skipped_nb = util.tableSize(self.selected_files)
    if util.tableSize(ok_files) > 0 then
        getReadHistory():removeItems(ok_files)
        getReadCollection():removeItems(ok_files)
        if skipped_nb > 0 then
            self:onRefresh()
        end
    end
    ...
    local function doRenameFile()
        if self:moveFile(file, dest) then
            if is_file then
                ...
                getReadHistory():updateItem(file, dest) -- (will update "lastfile" if needed)
                getReadCollection():updateItem(file, dest)
            else
                getReadHistory():updateItemsByPath(file, dest)
                getReadCollection():updateItemsByPath(file, dest)
            end
            self:onRefresh()
        else
            UIManager:show(getInfoMessage():new{
