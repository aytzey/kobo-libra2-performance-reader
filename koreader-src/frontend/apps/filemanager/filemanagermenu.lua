...
local function getKeyValuePage()
    KeyValuePage = KeyValuePage or require("ui/widget/keyvaluepage")
    return KeyValuePage
end

local function getSetDefaults()
    SetDefaults = SetDefaults or require("apps/filemanager/filemanagersetdefaults")
    return SetDefaults
end
...
- Calibre and OPDS browsers/search results]]),
                        callback = function(touchmenu_instance)
                            ...
                            local widget = getSpinWidget():new{
...
`Manga/Cells at Work`.]]),
            ...
            {
                text_func = function()
                    local nb_items_landscape, nb_items_portrait = getKeyValuePage().getCurrentItemsPerPage()
...
- A few other plugins]]),
                keep_menu_open = true,
                callback = function(touchmenu_instance)
                    local nb_items_landscape_default, nb_items_portrait_default = getKeyValuePage().getDefaultItemsPerPage()
                    local nb_items_landscape, nb_items_portrait =
                        getKeyValuePage().getCurrentItemsPerPage(nb_items_landscape_default, nb_items_portrait_default)
                    local widget = getDoubleSpinWidget():new{
...
function FileManagerMenu:getShowFilterMenuTable()
    ...
    local sub_item_table = {
        {
            text = getBookList().getBookStatusString("all"):lower(),
            checked_func = function()
    ...
    for _, v in ipairs(statuses) do
        table.insert(sub_item_table, {
            text = getBookList().getBookStatusString(v):lower(),
            checked_func = function()
            ...
            if FileChooser.show_filter.status == nil then
                ...
                for _, v in ipairs(statuses) do
                    if FileChooser.show_filter.status[v] then
                        local status_string = getBookList().getBookStatusString(v):lower()
