...
function FileSearcher:onShowFileSearch(search_string)
    ...
    check_button_subfolders = getCheckButton():new{
    ...
    if self.ui.coverbrowser then
        check_button_metadata = getCheckButton():new{
...
function FileSearcher:onMenuHold(item)
    ...
    if is_file then
        ...
        if getFileManagerConverter():isSupported(file) then
            table.insert(buttons, {
                getFileManagerConverter():genConvertButton(file, close_dialog_callback, close_menu_refresh_callback)
            })
        end
        table.insert(buttons, {
            {
