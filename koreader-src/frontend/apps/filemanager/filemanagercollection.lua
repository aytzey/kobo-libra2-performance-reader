...
local function getCheckButton()
    CheckButton = CheckButton or require("ui/widget/checkbutton")
    return CheckButton
end
...
local function getMenu()
    Menu = Menu or require("ui/widget/menu")
    return Menu
end

local function getReadCollection()
    ReadCollection = ReadCollection or require("readcollection")
    return ReadCollection
end

local function getSortWidget()
    SortWidget = SortWidget or require("ui/widget/sortwidget")
    return SortWidget
end
...
function FileManagerCollection:getCollectionTitle(collection_name)
    return collection_name == getReadCollection().default_collection_name
...
function FileManagerCollection:onShowColl(collection_name)
    collection_name = collection_name or getReadCollection().default_collection_name
    getReadCollection():updateCollectionFromFolder(collection_name, nil, true)
    -- This may be hijacked by CoverBrowser plugin and needs to be known as booklist_menu.
    self.booklist_menu = getBookList():new{
        name = "collections",
...
function FileManagerCollection:updateItemTable(item_table, focused_file)
    if item_table == nil then
        item_table = {}
        for _, item in pairs(getReadCollection().coll[self.booklist_menu.path]) do
...
function FileManagerCollection:isItemMatch(item)
    if self.match_table then
        if self.match_table.status then
            if self.match_table.status ~= getBookList().getBookStatus(item.file) then
                return false
            end
        end
...
function FileManagerCollection:getBookListTitle(item_table)
    ...
    if self.match_table then
        subtitle = {}
        if self.match_table.status then
            local status_string = getBookList().getBookStatusString(self.match_table.status, true)
...
function FileManagerCollection:onMenuHold(item)
    ...
    if is_currently_opened then
        ...
        been_opened = getBookList().hasBookBeenOpened(file)
        if been_opened then
            doc_settings_or_file = getBookList().getDocSettings(file)
            if not book_props then
    ...
    table.insert(buttons, {
        ...
        },
        {
            ...
            callback = function()
                ...
                getReadCollection():removeItem(file, self.path, true)
    ...
    self.file_dialog = getButtonDialog():new{
...
function FileManagerCollection:showSelectModeDialog()
                    ...
                    UIManager:show(getConfirmBox():new{
                        ...
                        ok_callback = function()
                            ...
                            for file in pairs(self.selected_files) do
                                getReadCollection():removeItem(file, collection_name, true)
                            end
        ...
        {
            {
                ...
                callback = function()
                    ...
                    local caller_callback = function(selected_collections)
                        ...
                        getReadCollection():addItemsMultiple(self.selected_files, selected_collections)
                        self.updated_collections[collection_name] = true
                        for file in pairs(self.selected_files) do
                            getReadCollection():removeItem(file, collection_name, true)
                        end
    ...
    select_dialog = getButtonDialog():new{
        title = title,
        title_align = "center",
        buttons = buttons,
    }
...
function FileManagerCollection:showCollDialog()
    ...
    local function genFilterByStatusButton(button_status)
        return {
            text = getBookList().getBookStatusString(button_status),
            ...
            callback = function()
                ...
                local path_chooser = PathChooser:new{
                    ...
                    onConfirm = function(file)
                        if not getReadCollection():isFileInCollection(file, collection_name) then
                            self.updated_collections[collection_name] = true
                            getReadCollection():addItem(file, collection_name)
    ...
    if self.ui.document then
        ...
        local is_in_collection = getReadCollection():isFileInCollection(file, collection_name)
        table.insert(buttons, {{
            text_func = function()
    ...
    coll_dialog = getButtonDialog():new{
        buttons = buttons,
    }
...
function FileManagerCollection:setCollate(collate_id, collate_reverse)
    local coll_settings = getReadCollection().coll_settings[self.booklist_menu.path]
    ...
    if collate_id then
        local collate = getBookList().collates[collate_id]
...
function FileManagerCollection:showArrangeBooksDialog()
    local collection_name = self.booklist_menu.path
    local coll_settings = getReadCollection().coll_settings[collection_name]
    ...
    local buttons = {
        ...
        {{
            ...
            callback = function()
                UIManager:close(arrange_dialog)
                local sort_widget
                sort_widget = getSortWidget():new{
                    title = _("Arrange books in collection"),
                    item_table = self.booklist_menu.item_table,
                    callback = function()
                        getReadCollection():updateCollectionOrder(collection_name, sort_widget.item_table)
                        self.updated_collections[collection_name] = true
    ...
    arrange_dialog = getButtonDialog():new{
        title = _("Sort by"),
        title_align = "center",
        buttons = buttons,
    }
...
function FileManagerCollection:addBooksFromFolder(include_subfolders)
    ...
    local path_chooser = PathChooser:new{
        ...
        onConfirm = function(folder)
            local count = getReadCollection():updateCollectionFromFolder(self.booklist_menu.path,
            ...
            UIManager:show(getInfoMessage():new{ text = text })
        end,
    }
    UIManager:show(path_chooser)
end
...
function FileManagerCollection:onShowCollList(file_or_selected_collections, caller_callback, no_dialog)
    ...
    if file_or_selected_collections ~= nil then -- select mode
        ...
        if type(file_or_selected_collections) == "string" then -- checkmark collections containing the file
            self.selected_collections = getReadCollection():getCollectionsWithFile(file_or_selected_collections)
        else
...
function FileManagerCollection:updateCollListItemTable(do_init, item_number)
    ...
    if do_init then
        item_table = {}
        for coll_name in pairs(getReadCollection().coll) do
            local mandatory
            ...
            table.insert(item_table, {
                ...
                mandatory = mandatory,
                name      = coll_name,
                order     = getReadCollection().coll_settings[coll_name].order,
            })
        end
        if #item_table > 1 then
...
function FileManagerCollection.getCollListItemMandatory(coll_name)
    ...
    local coll_nb = util.tableSize(getReadCollection().coll[coll_name])
    return marker and marker .. " " .. coll_nb or coll_nb
end

function FileManagerCollection.getCollMarker(coll_name)
    local coll_settings = getReadCollection().coll_settings[coll_name]
    local marker
...
function FileManagerCollection:onCollListHold(item)
    ...
    local buttons = {
        ...
        item.name ~= getReadCollection().default_collection_name and { -- Favorites non-editable
            {
    ...
    button_dialog = getButtonDialog():new{
        title = item.text,
        title_align = "center",
        buttons = buttons,
    }
...
function FileManagerCollection:showCollFilterDialog(item)
    ...
    local coll_settings = getReadCollection().coll_settings[coll_name]
    local input_dialog
    input_dialog = getInputDialog():new{
...
function FileManagerCollection:showCollFolderList(item)
    local coll_name = item.name
    self.coll_folder_list = getMenu():new{
        path = coll_name,
        title = item.text,
        subtitle = "",
        covers_fullscreen = true,
...
function FileManagerCollection:updateCollFolderListItemTable()
    local item_table = {}
    local folders = getReadCollection().coll_settings[self.coll_folder_list.path].folders
    if folders then
        ...
        {
            {
                ...
                callback = function()
                    ...
                    if coll_settings.folders[folder].subfolders then
                        ...
                        getReadCollection():updateCollectionFromFolder(coll_name)
                    end
                    self._manager:updateCollFolderListItemTable()
                end,
            },
        ...
        buttons = {
            ...
            {}, -- separator
            {
                ...
                },
                {
                    ...
                    callback = function()
                        UIManager:close(button_dialog)
                        for name in pairs(getReadCollection().coll) do
        ...
        buttons = {{
            ...
            },
            {
                ...
                callback = function()
                    ...
                    if getReadCollection().coll[new_name] then
...
function FileManagerCollection:addCollection()
    local editCallback = function(name)
        self.updated_collections[name] = true
        getReadCollection():addCollection(name)
        local mandatory
        if self.selected_collections then
            self.selected_collections[name] = true
        ...
        table.insert(self.coll_list.item_table, {
            ...
            mandatory = mandatory,
            name      = name,
            order     = getReadCollection().coll_settings[name].order,
        })
...
function FileManagerCollection:renameCollection(item)
    local editCallback = function(name)
        self.updated_collections[name] = true
        getReadCollection():renameCollection(item.name, name)
...
function FileManagerCollection:removeCollection(item)
    UIManager:show(getConfirmBox():new{
        ...
        ok_callback = function()
            ...
            getReadCollection():removeCollection(item.name)
...
function FileManagerCollection:sortCollections()
    ...
    sort_widget = getSortWidget():new{
        ...
        callback = function()
            ...
            getReadCollection():updateCollectionListOrder(sort_widget.item_table)
            self:updateCollListItemTable(true) -- init
        end,
    }
    UIManager:show(sort_widget)
...
function FileManagerCollection:onShowCollectionsSearchDialog(search_str, coll_name)
    ...
    search_dialog = getInputDialog():new{
    ...
    check_button_case = getCheckButton():new{
        text = _("Case sensitive"),
        checked = self.case_sensitive,
        parent = search_dialog,
    }
    search_dialog:addWidget(check_button_case)
    check_button_content = getCheckButton():new{
...
function FileManagerCollection:searchCollections(coll_name)
    ...
    local collections = coll_name and { [coll_name] = getReadCollection().coll[coll_name] } or getReadCollection().coll
    local Trapper = require("ui/trapper")
    local info = getInfoMessage():new{ text = _("Searching… (tap to cancel)") }
    UIManager:show(info)
    ...
    local completed, files_found, files_found_order = Trapper:dismissableRunInSubprocess(function()
        ...
        for collection_name, coll in pairs(collections) do
            local coll_order = getReadCollection().coll_settings[collection_name].order
    ...
    if #files_found_order == 0 then
        ...
        getReadCollection():removeCollection(new_coll_name)
        getReadCollection():addCollection(new_coll_name)
        getReadCollection():addItemsMultiple(files_found, { [new_coll_name] = true })
        getReadCollection():updateCollectionOrder(new_coll_name, files_found_order)
...
function FileManagerCollection:onCloseWidget()
    if next(self.updated_collections) then
        getReadCollection():write(self.updated_collections)
    end
end
        ...
        callback = function()
            ...
            local caller_callback = function(selected_collections)
                ...
                if is_single_file then
                    getReadCollection():addRemoveItemMultiple(file_or_files, selected_collections)
                else -- selected files
                    getReadCollection():addItemsMultiple(file_or_files, selected_collections)
                end
