...
function BookInfo:show(doc_settings_or_file, book_props)
    ...
    if not has_sidecar and getBookList().hasBookBeenOpened(file) then
    ...
    book_props.pages = book_props.pages or getBookList().getBookInfo(file).pages
    -- cover image
...
function BookInfo:getDocProps(file, book_props, no_open_document)
    ...
    if getBookList().hasBookBeenOpened(file) then
        local doc_settings = getBookList().getDocSettings(file)
...
function BookInfo:showBookProp(prop_key, prop_text)
    UIManager:show(getTextViewer():new{
        ...
        {
            {
                ...
                callback = function()
                    local confirm_box = getConfirmBox():new{
...
function BookInfo:showNotebookFileDialog(notebook_file, doc_settings_or_file, book_props)
    ...
    local function saveNotebookFile(new_notebook_file)
        if not has_sidecar then
            doc_settings_or_file = getBookList().getDocSettings(doc_settings_or_file)
        end
    ...
    local buttons = {
        {
            ...
            },
            {
                ...
                callback = function()
                    ...
                    getNotification():notify(_("Notebook file default location reset"), getNotification().SOURCE_ALWAYS_SHOW)
                end,
            },
        },
        {
            ...
            },
            {
                ...
                callback = function()
                    ...
                    getNotification():notify(_("Notebook file default location saved"), getNotification().SOURCE_ALWAYS_SHOW)
                end,
            },
        },
        {
        ...
        {
            {
                ...
                callback = function()
                    UIManager:close(button_dialog)
                    getTextViewer().openFile(notebook_file)
                end,
            },
            {
                text = _("Edit"),
...
%M current time (hh-mm-ss)]]),
    ...
    if file then
        if is_doc_required then
            ...
            elseif getBookList().hasBookBeenOpened(file) then -- do not open book, use sdr only
                local doc_settings = getBookList().getDocSettings(file)
                props = BookInfo.extendProps(doc_settings:readSetting("doc_props"), file)
                if patterns["%t"] or patterns["%c"] or patterns["%p"] then
                    local book_info = getBookList().getBookInfo(file)
