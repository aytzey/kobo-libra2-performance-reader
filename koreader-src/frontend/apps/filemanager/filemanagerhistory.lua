...
function FileManagerHistory:fetchStatuses(count)
    for _, v in ipairs(require("readhistory").hist) do
        ...
        elseif v.file == (self.ui.document and self.ui.document.file) then -- currently opened file
            ...
            status = getBookList().getBookStatus(v.file)
        end
        if count then
...
function FileManagerHistory:updateItemTable()
    ...
    for _, v in ipairs(require("readhistory").hist) do
        if self:isItemMatch(v) then
            ...
            if item.select_enabled and getReadCollection():isFileInCollections(item.file) then
...
function FileManagerHistory:isItemMatch(item)
    ...
    if self.selected_collections then
        for name in pairs(self.selected_collections) do
            if not getReadCollection():isFileInCollection(item.file, name) then
                return false
            end
        end
    end
...
function FileManagerHistory:getBookListTitle(item_table)
    ...
    elseif self.filter ~= "all" then
        subtitle = getBookList().getBookStatusString(self.filter, true)
    end
...
function FileManagerHistory:showHistDialog()
    ...
    local function genFilterButton(filter)
        return {
            text = T(_("%1 (%2)"), getBookList().getBookStatusString(filter), self.count[filter]),
            callback = function()
        ...
        table.insert(buttons, {
            {
                ...
                callback = function()
                    local confirmbox = getConfirmBox():new{
                        text = _("Clear history of deleted files?"),
                        ok_text = _("Clear"),
    ...
    hist_dialog = getButtonDialog():new{
