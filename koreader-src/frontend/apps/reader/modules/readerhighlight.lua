...
local function getButtonDialog()
    ButtonDialog = ButtonDialog or require("ui/widget/buttondialog")
    return ButtonDialog
end

local function getConfirmBox()
    ConfirmBox = ConfirmBox or require("ui/widget/confirmbox")
    return ConfirmBox
end

local function getDoubleSpinWidget()
    DoubleSpinWidget = DoubleSpinWidget or require("ui/widget/doublespinwidget")
...
local function getSpinWidget()
    SpinWidget = SpinWidget or require("ui/widget/spinwidget")
    return SpinWidget
end

local function getTextViewer()
    TextViewer = TextViewer or require("ui/widget/textviewer")
    return TextViewer
end

local function getTranslator()
    Translator = Translator or require("ui/translator")
    return Translator
end
        ...
        ["03_copy"] = function(this)
            return {
                ...
                callback = function()
                    ...
                    this:onClose(true)
                    UIManager:show(getNotification():new{
...
local lookup_highlight_actions = {
    translate = true,
    wikipedia = true,
    dictionary = true,
    search = true,
...
local lookup_highlight_buttons = {
    ["05_wikipedia"] = true,
    ["06_dictionary"] = true,
    ["07_translate"] = true,
    ["12_search"] = true,
...
local function lookupHighlightActionsDisabled()
    return G_reader_settings:isTrue("disable_highlight_lookup_actions")
end
...
local function isLookupHighlightAction(action)
    return lookup_highlight_actions[action] or false
end

local function normalizeDefaultHighlightAction(action)
    if lookupHighlightActionsDisabled() and isLookupHighlightAction(action) then
        return "ask"
    end
    return action
end

local function getHighlightActionNum(action)
...
function ReaderHighlight:addToMainMenu(menu_items)
    ...
    table.insert(hl_sub_item_table, {
        ...
        callback = function(touchmenu_instance)
            local spin_widget = getSpinWidget():new{
    ...
    table.insert(hl_sub_item_table, {
        ...
        callback = function()
            UIManager:show(getConfirmBox():new{
...
If you wish your highlights to be saved in the document, just move it to a writable directory first.]]),
    ...
    if not lookupHighlightActionsDisabled() then
    ...
    -- actions
    local long_press_action_items = {}
    for i, v in ipairs(long_press_action) do
        if not (lookupHighlightActionsDisabled() and isLookupHighlightAction(v[2])) then
            local action_item = {
                text = v[1],
                checked_func = function()
                    return normalizeDefaultHighlightAction(G_reader_settings:readSetting("default_highlight_action", "ask")) == v[2]
                end,
                radio = true,
                callback = function()
            ...
            table.insert(menu_items.long_press.sub_item_table, action_item)
            table.insert(long_press_action_items, action_item)
        end
    end
...
Except when in two columns mode, where this is limited to showing only the previous or next column.]]),
    ...
    if not Device:isTouchDevice() and Device:hasDPad() then
        table.insert(menu_items.long_press.sub_item_table, {
            ...
            callback = function(touchmenu_instance)
                local reader_speed = G_reader_settings:readSetting("highlight_non_touch_factor") or 4
                local dict_speed = G_reader_settings:readSetting("highlight_non_touch_factor_dict") or 3
                local double_spin_widget = getDoubleSpinWidget():new{
                    left_text = _("Reader"),
        ...
        local selection_text_sub_item_table = {}
        if not lookupHighlightActionsDisabled() then
            table.insert(selection_text_sub_item_table, menu_items.long_press.sub_item_table[1]) -- Dictionary on single word selection
        end
        table.insert(selection_text_sub_item_table, {
            text_func = function()
                local multi_word = normalizeDefaultHighlightAction(G_reader_settings:readSetting("default_highlight_action", "ask"))
            ...
            sub_item_table = long_press_action_items,
        })
        menu_items.selection_text = {
            text = _("Text selection tools"),
            sub_item_table = selection_text_sub_item_table,
        }
        local post_long_press_action_index = #long_press_action_items + 1 -- index after long_press_action
        if not lookupHighlightActionsDisabled() then
            post_long_press_action_index = post_long_press_action_index + 1 -- Dictionary on single word selection
        end
    ...
    if not lookupHighlightActionsDisabled() then
        menu_items.translation_settings = getTranslator():genSettingsMenu()
...
function ReaderHighlight:showHighlightDialog(index)
    ...
    edit_highlight_dialog = getButtonDialog():new{
...
function ReaderHighlight:onShowHighlightMenu(index)
    ...
    for idx, fn_button in ffiUtil.orderedPairs(self._highlight_buttons) do
        ...
        if not (lookupHighlightActionsDisabled() and lookup_highlight_buttons[idx])
                and (not button.show_in_highlight_dialog_func or button.show_in_highlight_dialog_func()) then
    ...
    self.highlight_dialog = getButtonDialog():new{
    ...
    if not clear then
        ...
        if self.is_word_selection then
            ...
            if G_reader_settings:isTrue("highlight_action_on_single_word") and
                   normalizeDefaultHighlightAction(G_reader_settings:readSetting("default_highlight_action", "ask")) == "ask" then
                handle_long_hold = false
            end
        else
            -- Multi words selection uses default_highlight_action, and no need for long-hold
            -- if it is already "ask".
            if normalizeDefaultHighlightAction(G_reader_settings:readSetting("default_highlight_action", "ask")) == "ask" then
                handle_long_hold = false
            end
        end
...
function ReaderHighlight:isOCRFallbackEnabled()
    local doc = self.ui and self.ui.document
    if doc and doc.koptinterface and doc.koptinterface.isOCRFallbackEnabled then
        return doc.koptinterface:isOCRFallbackEnabled(doc)
    end
    if doc and doc.configurable and doc.configurable.disable_ocr_fallback == 1 then
        return false
    end
    if isTrueSetting(G_reader_settings, "disable_ocr_fallback") then
        return false
    end
    return not isTrueSetting(G_defaults, "DKOPTREADER_DISABLE_OCR_FALLBACK")
end

function ReaderHighlight:showOCRUnavailableMessage()
    UIManager:show(getInfoMessage():new{
...
function ReaderHighlight:lookupDictWord()
    if lookupHighlightActionsDisabled() then return end
    ...
    elseif self.selected_text.sboxes then
        if not self:isOCRFallbackEnabled() then
            self:showOCRUnavailableMessage()
            return
        end
...
function ReaderHighlight:onTranslateText(text, index)
    getTranslator():showTranslation(text, true, nil, nil, true, index)
end
...
function ReaderHighlight:onTranslateCurrentPage()
    ...
    if res and res.text then
        getTranslator():showTranslation(res.text, false, self.ui.doc_props.language)
    end
end
...
function ReaderHighlight:onHoldRelease()
    ...
    local default_highlight_action = normalizeDefaultHighlightAction(G_reader_settings:readSetting("default_highlight_action", "ask"))
    ...
    if self.is_word_selection then -- single-word selection
        if lookupHighlightActionsDisabled()
                or long_final_hold
                or G_reader_settings:isTrue("highlight_action_on_single_word") then
            self.is_word_selection = false
        end
    end

    if self.selected_text then
        if self.is_word_selection and not lookupHighlightActionsDisabled() then
            self:lookupDictWord()
        else
...
function ReaderHighlight:onSetHighlightAction(action_num, no_notification)
    ...
    if lookupHighlightActionsDisabled() and v and isLookupHighlightAction(v[2]) then
        action_num = getHighlightActionNum("ask")
        v = long_press_action[action_num]
    end
...
function ReaderHighlight:onCycleHighlightAction()
    ...
    local current_action_num = 0
    for i, v in ipairs(long_press_action) do
        if v[2] == current_action then
            current_action_num = i
            break
        end
    end
    for offset = 1, #long_press_action do
        local next_action_num = current_action_num + offset
        if next_action_num > #long_press_action then
            next_action_num = next_action_num - #long_press_action
        end
        local v = long_press_action[next_action_num]
        if not (lookupHighlightActionsDisabled() and isLookupHighlightAction(v[2])) then
...
function ReaderHighlight:onReadSettings(config)
    ...
    self.view.highlight.disabled = normalizeDefaultHighlightAction(G_reader_settings:readSetting("default_highlight_action")) == "nothing"
