...
function ReaderZooming:getZoom(pageno)
    ...
    local reserve = self.view and self.view.getBottomReserve and self.view:getBottomReserve() or 0
    local zoom_h = math.max(1, self.dimen.h - reserve)
    if self.ui.view.footer_visible and not self.ui.view.footer.settings.reclaim_height then
        zoom_h = math.max(1, zoom_h - self.ui.view.footer:getHeight())
    end
