...
function ReaderView:getBottomReserve()
    local reserve = tonumber(self.ui and self.ui.ttsreader_bottom_reserve) or 0
    if reserve <= 0 then
        return 0
    end
    return math.min(math.floor(reserve), math.max(0, self.dimen.h - 1))
end

function ReaderView:getViewportDimen()
    return Geom:new{
        w = self.dimen.w,
        h = math.max(1, self.dimen.h - self:getBottomReserve()),
    }
end
...
function ReaderView:drawXPointerSavedHighlight(bb, x, y)
    ...
    if self.highlight.page_boxes[page] ~= nil then -- cached
        ...
        local viewport_h = self:getViewportDimen().h
        if self.view_mode == "page" and self.document:getVisiblePageCount() > 1 then
            cur_view_bottom = cur_view_top + 2 * viewport_h
        else
            cur_view_bottom = cur_view_top + viewport_h
        end
...
function ReaderView:recalculate()
    ...
    local viewport = self:getViewportDimen()
    ...
    if self.ui.paging and self.state.page then
        self.page_area = self:getPageArea(
            ...
            self.state.rotation)
        -- reset our size
        self.visible_area:setSizeTo(viewport)
    ...
    if viewport.h > self.visible_area.h then
        if self.footer_visible and not self.footer.settings.reclaim_height then
            self.state.offset.y = (viewport.h - (self.visible_area.h + self.footer:getHeight())) / 2
        else
            self.state.offset.y = (viewport.h - self.visible_area.h) / 2
        end
    end
    if self.dimen.w > self.visible_area.w then
