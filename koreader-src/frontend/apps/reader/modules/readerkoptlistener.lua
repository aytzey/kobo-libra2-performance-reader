...
local function getOCRUtil()
    OCRUtil = OCRUtil or require("ui/data/ocr")
    return OCRUtil
end

function ReaderKoptListener:isOCRFallbackEnabled()
...
function ReaderKoptListener:onReadSettings(config)
    ...
    -- Auto-select OCR language only when OCR fallback is actually available.
    if self:isOCRFallbackEnabled() then
        local available = getOCRUtil().getOCRLangs()
        if #available > 0 then
