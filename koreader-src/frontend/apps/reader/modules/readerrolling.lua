...
local function getDocumentViewport(self)
    if self.view and self.view.getViewportDimen then
        return self.view:getViewportDimen()
    end
...
function ReaderRolling:onGotoViewRel(diff)
    ...
    if self.view.view_mode == "scroll" then
        ...
        local viewport = getDocumentViewport(self)
        local page_visible_height = viewport.h - footer_height
...
function ReaderRolling:onSetDimensions(dimen)
    if self.ui.postReaderReadyCallback ~= nil then
        ...
        self.ui.document:setViewDimen(getDocumentViewport(self))
...
function ReaderRolling:_gotoPos(new_pos, do_dim_area)
    ...
    local viewport = getDocumentViewport(self)
    local max_pos = self.ui.document.info.doc_height - viewport.h + self.view.footer:getHeight()
