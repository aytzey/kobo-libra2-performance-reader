...
local function loadPendingPluginMeta(plugin)
    if not plugin._metadata_pending then return plugin end

    local package_path = package.path
    local package_cpath = package.cpath
    package.path = string.format("%s/?.lua;%s", plugin.path, package_path)
    package.cpath = string.format("%s/lib/?.so;%s", plugin.path, package_cpath)

    local ok_meta, plugin_metamodule = pcall(dofile, plugin._metadata_pending)
    if ok_meta and plugin_metamodule then
        for k, module in pairs(plugin_metamodule) do
            plugin[k] = module
        end
    else
        logger.warn("Error when loading", plugin._metadata_pending, plugin_metamodule)
    end

    package.path = package_path
    package.cpath = package_cpath
    plugin._metadata_pending = nil
    return plugin
end

local function makeLazyDisabledPlugin(plugin)
    return {
        path = plugin.path,
        name = plugin.id,
        fullname = plugin.id,
        description = "",
        _metadata_pending = plugin.meta,
    }
end

-- Deprecated plugins are still available, but show a hint about deprecation.
local function getMenuTable(plugin)
    plugin = loadPendingPluginMeta(plugin)
    local t = {}
    t.name = plugin.name
    ...
    for _, lookup_path in ipairs(lookup_path_list) do
        ...
        for entry in lfs.dir(lookup_path) do
            ...
            if mode == "directory" and entry:sub(-9) == ".koplugin" then
                ...
                local plugin_id = entry:sub(1, -10)
                if plugins_disabled and plugins_disabled[plugin_id] then
                    mainfile = metafile
                    disabled = true
                end
...
function PluginLoader:_load(t)
    ...
    for _, v in ipairs(t) do
        ...
        if disabled and G_reader_settings:isTrue("libra2_lazy_disabled_plugin_meta") then
            table.insert(self.disabled_plugins, makeLazyDisabledPlugin(v))
        else
            ...
            elseif type(plugin_module.disabled) ~= "boolean" or not plugin_module.disabled then
                ...
                plugin_module.name = plugin_module.name or v.id or plugin_root:match("/(.-)%.koplugin")
                if disabled then
